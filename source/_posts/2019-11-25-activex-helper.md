...
|                  |         |  |--------------|   |----------|   |                |
|  |------------|  |         |  |     小型      |   | ActiveX  |  |     身份证       |
|  | JavaScript |-----AJAX------|  Web Server  |---|   控件    |--------阅读器-----------身份证阅读器
|  |------------|  |         |  |--------------|   |----------|   |     驱动        |
|                  |         |                                    |                |
|------------------|         |------------------------------------|                |
```
...
代码中的监听端口为12345。为了避免冲突，可以修改成其他端口（最大65535，尽量用五位数）。由于监听地址为127.0.0.1（本机），业务系统调用时为跨域请求，需要通过JSONP方式调用，所以小助手这边需要以GET方式提供服务，并返回JSON格式数据，不能使用POST。

## Web应用页面代码调整
调整读卡按钮的逻辑，先判断ActiveX控件是否可用。如果可用，沿用原先的ActiveX操作方式；如果不可用，则尝试通过$.ajax向“小助手”的小型Web Server发送请求。

调用“小助手”属于跨域请求，需要通过JSONP方式来发起请求。
...
# 投入商业使用之前
以上是实验用原型，质量粗糙。交付用户之前，还需要一系列优化，例如：
